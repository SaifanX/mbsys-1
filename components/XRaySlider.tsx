...
const XRaySlider: React.FC<XRaySliderProps> = ({ 
  ...
  return (
    <div 
      ref={containerRef}
      className="relative w-full aspect-[4/5] md:aspect-video lg:h-[600px] overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xl select-none group cursor-ew-resize transition-all duration-700 touch-none bg-slate-100 dark:bg-slate-900 gpu-accelerated"
      onMouseDown={handleMouseDown}
      onTouchStart={handleTouchStart}
    >
      {/* Loading Overlay */}
      <div className={`absolute inset-0 z-50 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900 transition-opacity duration-700 ${isReady ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      ...
      <div className={`absolute inset-0 w-full h-full bg-slate-900 overflow-hidden transition-opacity duration-1000 ${isReady ? 'opacity-100' : 'opacity-0'} gpu-accelerated`}>
         {!loadError.after ? (
           <img 
            src={imageAfter} 
        ...
        <div className="absolute top-4 left-4 sm:top-8 sm:left-8 bg-secondary/90 backdrop-blur-xl px-4 py-2 rounded-lg border border-white/20 text-white text-[10px] font-bold uppercase tracking-[0.2em] shadow-2xl z-10 transition-transform group-hover:scale-105">
           {labelAfter}
        </div>
      </div>
      ...
      <div 
        className={`absolute inset-0 w-full h-full z-20 transition-opacity duration-1000 ${isReady ? 'opacity-100' : 'opacity-0'} gpu-accelerated`}
        style={{ 
      ...
      <div 
        className={`absolute top-0 bottom-0 w-[2px] bg-primary cursor-ew-resize z-30 shadow-[0_0_30px_rgba(239,68,68,1)] transition-opacity duration-700 ${isReady ? 'opacity-100' : 'opacity-0'} gpu-accelerated`}
