...
const QuickActionHub: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[200] flex flex-col items-end gap-3 md:hidden">
      {/* Action Menu - Mobile Tactile Stack */}
      <div className={`flex flex-col gap-3 transition-all duration-[700ms] ease-[cubic-bezier(0.32,0.72,0,1)] origin-bottom ${isOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-75 opacity-0 translate-y-12 pointer-events-none'}`}>
        {actions.map((action, idx) => (
          <div key={idx} className="flex items-center gap-3 group">
            <div className="bg-white/95 dark:bg-slate-800/95 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl backdrop-blur-md">
